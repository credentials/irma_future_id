...
public class PINStepAction extends StepAction {
	...
	/* "0024000110" << old_pin_hex.str() << "0000" << new_pin_hex.str() << "0000"; */

	byte[] apdu_1 = {(byte) 0x00, (byte) 0x24, (byte) 0x00, (byte) 0x01, (byte) 0x10};
	byte[] apdu_2 = {(byte) 0x00, (byte) 0x00};
	
	byte[] apduUpdateAdmin = new byte[apdu_1.length + 6*2 + apdu_2.length*2];
	...
	System.arraycopy(apdu_1, 0, apduUpdateAdmin, 0, apdu_1.length);
	System.arraycopy(old_pin_hex_byte, 0, apduUpdateAdmin, apdu_1.length, old_pin_hex_byte.length);
	System.arraycopy(apdu_2, 0, apduUpdateAdmin, apdu_1.length + old_pin_hex_byte.length, apdu_2.length);
	System.arraycopy(new_pin_hex_byte, 0, apduUpdateAdmin, apdu_1.length + old_pin_hex_byte.length + apdu_2.length, new_pin_hex_byte.length);
	System.arraycopy(apdu_2, 0, apduUpdateAdmin, apdu_1.length + old_pin_hex_byte.length + apdu_2.length + new_pin_hex_byte.length, apdu_2.length);
	    ...
	    inputAPDU.setInputAPDU(apduUpdateAdmin);
