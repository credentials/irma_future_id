...
import org.openecard.sal.protocol.irmacredential.anytype.IRMACREDENTIALDIDAuthenticateInputType;
import org.openecard.sal.protocol.irmacredential.anytype.IRMACREDENTIALDIDAuthenticateOutputType;
...
public class DIDAuthenticateStep implements ProtocolStep<DIDAuthenticate, DIDAuthenticateResponse> {
	...
	try {
	    ...
	    IRMACREDENTIALDIDAuthenticateInputType pinCompareInput = new IRMACREDENTIALDIDAuthenticateInputType(request.getAuthenticationProtocolData());
	    IRMACREDENTIALDIDAuthenticateOutputType pinCompareOutput = pinCompareInput.getOutputType();
		...
		responseCode = transResp.getOutputAPDU().get(0);
                ...
                byte[] template_get_cred = new byte[] { (byte) 0x80, (byte) 0x3A, (byte) 0x00, (byte) 0x00 };
                
                Transmit getCredentialsTransmit = IRMAUtils.buildGetCredentials(template_get_cred, slotHandle);
                TransmitResponse transRespGetCredentials = (TransmitResponse) dispatcher.deliver(getCredentialsTransmit);
                
                byte[] data = transRespGetCredentials.getOutputAPDU().get(0);
                
                data = Arrays.copyOfRange(data, 0, data.length - 2); // remove 0x9000    
            ...
            pinCompareOutput.setCredList(list);
