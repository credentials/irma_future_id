 ...
 */
public class IRMAProverMarkerTypeTest {

    private static final byte[] rootApplication = StringUtils.toByteArray("F849524D4163617264");
    private static final String cardType = "https://www.irmacard.org/irma-card";
    private static final String didName = "IRMA.PROVER";
	...
	IRMAProverMarkerType marker = new IRMAProverMarkerType(
		 didInfoWrapper.getDifferentialIdentity().getDIDMarker().getIRMAProverMarker());

	assertEquals(marker.getPolicy(), "default");
	assertEquals(marker.getProtocol(), ECardConstants.Protocol.IRMA_PROVER);
    }
}
