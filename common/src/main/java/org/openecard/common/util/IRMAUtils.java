...
*/
public class IRMAUtils {

    private static final Logger logger = LoggerFactory.getLogger(IRMAUtils.class);
	...
	// concatenate template with encoded pin
    	byte[] pin = IRMAUtils.encodePin(rawPIN, attributes);
    ...
    public static Transmit buildGetLog(byte[] template, byte index_1, byte index_2, byte[] slotHandle) throws UtilException {

	byte[] pinCmd = ByteUtils.concatenate(template, (byte) index_1);
	pinCmd = ByteUtils.concatenate(pinCmd, (byte) index_2);
    ...
    public static Transmit buildGetCredentials(byte[] template, byte[] slotHandle) throws UtilException {

	Transmit transmit = new Transmit();
	transmit.setSlotHandle(slotHandle);
	InputAPDUInfoType pinApdu = new InputAPDUInfoType();
	pinApdu.setInputAPDU(template);
	pinApdu.getAcceptableStatusCode().add(new byte[] {(byte)0x90, (byte)0x00});
	transmit.getInputAPDUInfo().add(pinApdu);
	return transmit;
    }
	...
	// concatenate template with encoded pin
    	...
    	byte[] oldPin = IRMAUtils.encodePin(rawOldPIN, attributes);
    	byte[] newPin = IRMAUtils.encodePin(rawPIN, attributes);
